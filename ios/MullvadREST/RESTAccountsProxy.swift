...
extension REST {
    public final class AccountsProxy: Proxy<AuthProxyConfiguration>, RESTAccountHandling {
        public init(configuration: AuthProxyConfiguration) {
            ...
            let responseHandler = AnyResponseHandler<NewAccountData> { response, data in
                // Print raw JSON response for debugging
                if let rawJSONString = String(data: data, encoding: .utf8) {
                    print("Raw JSON response: \(rawJSONString)")
                }

                // Check for successful HTTP status code and attempt decoding
                if HTTPStatus.isSuccess(response.statusCode) {
                    do {
                        let decodedData = try self.responseDecoder.decode(NewAccountData.self, from: data)
                        return .success(decodedData)
                    } catch {
                        // Handle decoding error
                        // Since there's no 'failure' case, we use 'unhandledResponse'
                        // with optional server error response if available
